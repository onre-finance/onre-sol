...
pub fn get_nav_adjustment(ctx: Context<GetNavAdjustment>) -> Result<i64> {
    ...
    // Preserve the signed external API while reusing the shared magnitude computation.
    let (previous_price_opt, adjustment) = if let Some(previous_vector) =
        find_previous_vector(&offer, active_vector.start_time)
    {
        ...
        let shared_adjustment =
            calculate_market_stats_nav_adjustment(&offer, active_vector, active_vector.start_time)?;
        let signed_adjustment = if current_price >= previous_price {
            shared_adjustment as i64
        } else {
            -(shared_adjustment as i64)
        };

        (Some(previous_price), signed_adjustment)
    } else {
        let shared_adjustment =
            calculate_market_stats_nav_adjustment(&offer, active_vector, active_vector.start_time)?;
