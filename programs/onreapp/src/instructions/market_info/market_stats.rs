...
pub fn recompute_market_stats(
    ...
    let nav_adjustment = calculate_nav_adjustment(offer, active_vector, current_time)?;
...
pub fn calculate_nav_adjustment(
    ...
    let adjustment =
        if let Some(previous_vector) = find_previous_vector(offer, active_vector.start_time) {
            ...
            current_price.abs_diff(previous_price)
        } else {
            current_price
        };
...
mod tests {
    ...
    #[test]
    fn nav_adjustment_uses_absolute_difference_for_negative_moves() {
        ...
        let adjustment = calculate_nav_adjustment(&offer, current, 200).unwrap();
    ...
    fn first_vector_adjustment_matches_current_nav() {
        ...
        let adjustment = calculate_nav_adjustment(&offer, current, 100).unwrap();
