...
/// Account structure for depositing tokens to the dual redemption vault.
///
/// This struct defines the accounts required for the boss to deposit tokens
/// into the dual redemption vault authority's token accounts.
#[derive(Accounts)]
pub struct DualRedemptionVaultDeposit<'info> {
...
///
/// Transfers tokens from the boss's token account to the dual redemption vault's token account
