...
async function createMakeOfferOneTransaction() {
    ...
    const sellTokenStartAmount = new BN('20279434000000');     // 6 decimals for USDC
    const sellTokenEndAmount = new BN('20307444000000');       // 6 decimals for USDC
    const offerStartTime = Math.floor(new Date(2025, 6, 5).getTime() / 1000); // July 5, 2025
    const offerEndTime = offerStartTime + (60 * 60 * 24 * 10); // +10 days (July 15th)
