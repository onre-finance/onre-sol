...
// This function only applies for BUY Offers with a sell token of USDC and a buy token with 9 decimals
function calculateOfferPrice(
  offerAccount: Awaited<ReturnType<Program<OnreApp>["account"]["offer"]["all"]>>[0]
): { price: number; validUntil: number } {
  const buyTokenDecimals: number = 9;
  const sellTokenDecimals: number = 6;
  const offer = offerAccount.account;
  ...
  const currentUTime = Math.floor(Date.now() / 1000);
