...
import { decodeEventNonceFromMessageV2, getDepositForBurnPdasV2, getMessageTransmitterAccount, getProgramsV2, getReceiveMessagePdasV2 } from "./utils-v2";
...
export const depositForBurnSol = async (
  ...
  const destinationDomain = 0; // ETH_DOMAIN_ID
  const mintRecipient = new PublicKey(
    getBytes(evmAddressToBytes32(process.env.ETH_RECIPIENT_ADDRESS!))
  );
...
export const depositForBurnSolWithHook = async (
  ...
  const destinationDomain = Number(0); // ETH_DOMAIN_ID
  const mintRecipient = new PublicKey(
    getBytes(evmAddressToBytes32(process.env.ETH_RECIPIENT_ADDRESS!))
  );
  const pdas = getDepositForBurnPdasV2(
...
export const receiveMessageSol = async (
  ...
  const usdcAddress = new PublicKey(process.env.SOL_USDC_ADDRESS!);
  const userTokenAccount = new PublicKey(process.env.USER_TOKEN_ACCOUNT!);
  const remoteTokenAddressHex = process.env.ETH_USDC_ADDRESS!;
  const remoteDomain = '0'; // ETH_DOMAIN_ID
