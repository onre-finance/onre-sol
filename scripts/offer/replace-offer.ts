...
async function createReplaceOfferTransaction() {
    ...
    try {
        ...
        const closeIx = await helper.buildCloseOfferIx({
            tokenInMint: TOKEN_IN_MINT,
            tokenOutMint: TOKEN_OUT_MINT
        });
        const closeOfferTx = await helper.prepareTransaction(new Transaction().add(closeIx));

        const closeBase58 = helper.printTransaction(closeOfferTx, "Close Old Offer Transaction");
        ...
        const makeOfferTx = await helper.prepareTransaction(new Transaction().add(makeOfferIx));

        const makeBase58 = helper.printTransaction(makeOfferTx, "Make New Offer Transaction");
